...
const InviteWorkspaceModal: FC<IModal> = ({ show, onCloseModal, showPrev }) => {
  ...
  const onSubmit = useCallback((data: IForm) => {
    axios
      .post(`/api/workspaces/${workspaceParams}/members`, data, { withCredentials: true })
      .then((res) => {
  ...
  return (
    <Modal show={show} onCloseModal={onClose}>
      <ModalContent>
        <h1>{userData?.Workspaces.find((v) => v.url === workspaceParams)?.name}(으)로 사용자 초대</h1>
